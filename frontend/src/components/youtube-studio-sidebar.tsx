...
export function YouTubeStudioSidebar({ isOpen }: YouTubeStudioSidebarProps) {
  ...
  return (
    <>
      {/* Sidebar */}
      <div className={`fixed left-0 top-14 h-[calc(100vh-3.5rem)] bg-[#0f0f0f] border-r border-gray-800 transition-all duration-300 z-40 flex flex-col ${
        isOpen ? 'w-64' : 'w-16'
      }`}>
        {/* Main content area */}
        ...
        <div className="border-t border-gray-800 p-4 flex-shrink-0">
          <button className={`w-full flex items-center ${isOpen ? 'space-x-3 px-3' : 'justify-center px-2'} py-2 rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white transition-colors`}>
